...
void displayGraphGUI_M4(Node** graph, int N, int** paths, int* path_lens, pid_t* pids, int num_travelers) {
    const int screenWidth = 1000;
    const int screenHeight = 600;
    InitWindow(screenWidth, screenHeight, "Communication Network - Multi-Process");
    SetTargetFPS(60);
        ...
        if (travelers[i].path_len > 1) {
            ...
            while (temp) {
                if (temp->vertex == travelers[i].path[1]) {
                    travelers[i].current_edge_weight = temp->weight; break;
                }
                temp = temp->next;
            }
    ...
    while (!WindowShouldClose()) {
                ...
                if (!travelers[i].isFinished) {
                    ...
                    if (travelers[i].state == 1) { // Moving
                        ...
                        if (travelers[i].state_timer >= 1.0f) {
                            travelers[i].state_timer -= 1.0f; travelers[i].state = 1;
                        }
                    }
                }
            }
        ...
        for (int u = 0; u < N; u++) {
            ...
            while (temp) {
                ...
                Vector2 p1 = { tip.x - arrow_length * cos(angle - PI/6), tip.y - arrow_length * sin(angle - PI/6) };
                Vector2 p2 = { tip.x - arrow_length * cos(angle + PI/6), tip.y - arrow_length * sin(angle + PI/6) };
                DrawLineEx(tip, p1, 2.0f, DARKGRAY); DrawLineEx(tip, p2, 2.0f, DARKGRAY);

                int midX = (start.x + end.x) / 2; int midY = (start.y + end.y) / 2;
        ...
        for (int i = 0; i < num_travelers; i++) {
            if (travelers[i].path_len > 0) {
                ...
                // Add a slight visual offset so concurrent dots don't perfectly hide each other on the same node
